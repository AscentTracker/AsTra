...
22.11.2015
 -d 1 to set daylight savings time

23.5.2015
 -stats
 -shorten beeps and init delay

26.1.2015
 -fixed: idle poweroff entered when ascent longer than 10 minutes was marked as failed (introduced B_NONE) 
...
1.05
 -show daily stats at climber selection
...
// As of IDE 1.1 no more code space for serial debugging, pity
#define S(a) ;
...
void statdump(void) {
    ...
    Serial.println(stats[climber].ascents);
    Serial.println(stats[climber].meters);
    Serial.println(stats[climber].seconds);
...
void setup() {
  ...
  stats = (Stat *)malloc (sizeof (Stat) * numclimbers);
  if (!stats) {
      error(mallocFailed);
  }

  for (byte i=0; i<numclimbers; i++) {
    stats[i].ascents=0;
    stats[i].meters=0.0;
    stats[i].seconds=0;
  };  
 ...
 if (initloop < MOVAVG_SIZE) {
   ...
   */
   ...
   return;
    ...
    // Timestamps, lazy DST handling
    sprintf(stime, "%02d:%02d:%02d", now.hour()-dst, now.minute(), now.second());
 ...
 switch (button) {  
   case B_START:
     ...
     switch (state) {
       ...
       case S_SELECTCLIMBER:    setZero(); state=S_IDLE; 
#if HWLCD
            LCD.setCursor(LCD_R1L);
            LCD.print(gradenum2name(grade,str));
   ...
   case B_MENU: // mark ascent aborted
     switch (state) {
       case S_CLIMBING: failed=true; button=B_NONE; state=S_POSTCLIMBING; break;
       case S_IDLE: button=B_NONE; state=S_SELECTCLIMBER; break;
       case S_SELECTCLIMBER:
             button=B_NONE;
             climber++;
...
switch (state) {
  case S_IDLE:
   ...
   //  -10 seconds after last re-set when after init and not waiting to climb over autostart level
    ...
    if ((button || (mnow < 5000) || 
          ((mnow > 5000)  && (mnow-zerotime>30000) && (start_stop_autotimer==0)))) {
     setZero();
     zerotime=mnow;
    };
 ...
 case S_POSTCLIMBING:
    ...
    buzz(piezoPin, FR1, 500, 50);
    endtime=mnow;

     if (!failed) {
      // Update per climber statistics for this session
        stats[climber].ascents++;
        stats[climber].meters+=maxalt-zero;
        stats[climber].seconds+=(endtime-starttime)/1000;
    }
...
// ============================================
  ...
  if (((mnow-lastprint)>INTERVAL)) {
    ...
    } else if (state == S_SELECTCLIMBER) { // show climber statistics
      LCD.setCursor(LCD_R1L);
      LCD.write(0xf6); // sigma
      LCD.print(F(" "));
      // LCD.print(F("Total "));
      LCD.print(stats[climber].ascents);
      LCD.print(F("  "));
      LCD.setCursor(LCD_R2L);
      LCD.print(stats[climber].meters,1);
      // LCD.print(FS(Spaces4));
      LCD.print(FS(SpaceMSpace));
      LCD.setCursor(LCD_R2R);      
      LCD.print(stats[climber].seconds);
      LCD.print(FS(SpaceS));
      LCD.print(FS(Spaces4));
      LCD.print(F("  "));
